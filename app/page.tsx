            ...
            <div className="grid place-items-center h-[calc(100vh-var(--global-nav-collective-height))] text-primary">
                <div className="text-5xl md:text-7xl lg:text-9xl text-center font-bold text-nowrap">
                    ...
                    <div className="relative" onClick={() => {
                        ...
                        <Dots key={dotsKey} width={dotsDimensions.width} height={dotsDimensions.height} density={(dotsDimensions.width < 700) ? 200 : 500}>
                            <h1 ref={h1Ref} className={`${(spoilerOpacity) ? ("opacity-100") : ("opacity-0")} transition-opacity whitespace-nowrap`}>
                                Sviatoslav Murzin</h1>
                        </Dots>
                        <h1 className={`text-sm md:text-lg lg:text-xl font-normal ${(spoilerOpacity) ? ("opacity-0") : ("opacity-100")}`}>(click here)</h1>
                    </div>
                    <div className="grid place-items-center">
                        <a href="https://github.com/securesvet"><FaGithubAlt className="hover:opacity-85 hover:cursor-pointer"/></a>
                    </div>
                </div>
