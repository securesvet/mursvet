...
const BackgroundImage = ({ imageUrl, className }: BackgroundImageProps) => {
  return (
    <div
      className={twMerge(
        "w-full h-full rounded-[inherit] box-border relative",
        className,
      )}
    >
      {imageUrl
        ? (
          <img
            src={imageUrl}
            className="w-full h-full object-cover rounded-[inherit] opacity-50 hover:opacity-25 transition-opacity hover:cursor-pointer"
          />
        )
        : (
